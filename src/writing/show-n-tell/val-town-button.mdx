...
<Callout>
  I wrote this show 'n' tell in November 2023 about the first val I ever made on
  Val Town. That was around when I wrote that I hoped [my next, next
  job](/my-next-next-next-job) would be at Val Town and two years before I ended
  up [joining Val Town](/december-2025#joining-val-town). I've since refactored
  the original val because vals are now versioned folders of code instead of
  single files, but I'm preserving this blog post as a snapshot of that moment
  in time.
</Callout>
...
```js
export let valTownButtonClicks = 377;
```

And here are the Vals that fetch and increment the count:

```js
import { valTownButtonClicks } from "https://esm.town/v/petermillspaugh/valTownButtonClicks";

export async function getValTownButtonClicks(request: Request): Promise<Response> {
  return Response.json({ valTownButtonClicks });
}
```

```js
import { valTownButtonClicks } from "https://esm.town/v/petermillspaugh/valTownButtonClicks";
import { set } from "https://esm.town/v/std/set?v=14";

export async function setValTownButtonClicks(request: Request): Promise<Response> {
  if (request.method !== "POST") return Response.json({ valTownButtonClicks });

  const result = await set("valTownButtonClicks", valTownButtonClicks + 1);
  return Response.json({ valTownButtonClicks: result });
}
```
